...
int myi2cRead(uint8_t addr, uint8_t reg, uint8_t len, uint8_t *buf)
{
   return siic->read_reg(addr<<1,reg,buf,len,200);
}

int myi2cWrite(uint8_t addr, uint8_t reg, uint8_t len, uint8_t *data)
{
    return siic->write_reg(addr<<1,reg,data,len,200);
}
...
#define i2c_write   myi2cWrite
#define i2c_read    myi2cRead
#define delay_ms    mydelay_ms
